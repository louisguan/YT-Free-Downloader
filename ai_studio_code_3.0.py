...
import os
import locale
import webbrowser # <--- 新增，用於打開網頁連結
from tkinter import font as tkFont # <--- 新增，用於設定字體
...
class DownloaderApp:
    def __init__(self, root):
        ...
        self.root.geometry("750x680") # 稍微增加高度以容納作者資訊
        ...
        self.author_url = "https://www.facebook.com/guan.shuai.2025/"
        author_label = tk.Label(author_frame, text=f"關於作者: {self.author_url}", fg="blue", cursor="hand2")
        author_label.pack(side=tk.RIGHT)
        
        # 設定底線字體
        underline_font = tkFont.Font(author_label, author_label.cget("font"))
        underline_font.configure(underline=True)
        author_label.configure(font=underline_font)
        
        # 綁定點擊事件
        author_label.bind("<Button-1>", self.open_author_link)
    ...
    def open_author_link(self, event):
        """在瀏覽器中打開作者連結"""
        webbrowser.open_new(self.author_url)

    # ... (此處之後的所有函數與前一版完全相同，保持不變) ...
